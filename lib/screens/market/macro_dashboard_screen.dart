...
class MacroDashboardScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<MacroDashboardScreen> createState() => _MacroDashboardScreenState();
}
...
class _MacroDashboardScreenState extends ConsumerState<MacroDashboardScreen> {
  ...
  Future<void> _loadAll() async {
    ...
    ref.read(usageAnalyticsProvider)?.logFeatureUsed(UsageFeature.macroDashboard);
    try {
      ...
      if (mounted) setState(() { _loading = false; _error = 'Failed to load macro data.'; });
    }
  }

  // ── Helpers ────────────────────────────────────────────────────────────────

  String _sectionHeader(String title) => title;  // used inline
      ...
      child: Column(
        ...
        children: [
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text('/ 100', style: const TextStyle(color: _textDim, fontSize: 12)),
                      ],
                    ),
                  ),
                ),
          ...
          Row(
            ...
            children: [
              Text('Extreme Fear', style: TextStyle(color: _fearGreedColor(10), fontSize: 10)),
              Text('Fear',         style: TextStyle(color: _fearGreedColor(30), fontSize: 10)),
              Text('Neutral',      style: TextStyle(color: _fearGreedColor(50), fontSize: 10)),
              Text('Greed',        style: TextStyle(color: _fearGreedColor(70), fontSize: 10)),
              Text('Extreme Greed',style: TextStyle(color: _fearGreedColor(90), fontSize: 10)),
            ],
          ),
          const SizedBox(height: 16),
          // Component breakdown
              ...
              return Padding(
                ...
                child: Row(
                  children: [
                    ...
                    const SizedBox(width: 8),
                    Text('${compScore.toStringAsFixed(0)}',
                        style: TextStyle(color: compColor, fontSize: 11,
  ...
  Widget _macroRegimeCard() {
    ...
    final regime    = _macroRegime!['regime'] as String?
    ...
    final rationale = _macroRegime!['rationale']  as String?
        ...
        children: [
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Text(
                      regime.replaceAll('_', ' '),
                      style: TextStyle(color: regColor, fontWeight: FontWeight.w700, fontSize: 12),
                    ),
                  ],
                ),
              ),
          ...
          if (driftAdj != null || volAdj != null) ...[
            ...
            Row(
              children: [
                if (driftAdj != null)
                  Expanded(child: _regimeStat(
                    'Drift Adj.',
                    '${driftAdj.toStringAsFixed(2)}×',
                    tooltip: 'Monte Carlo drift multiplier. 1.00× = neutral; >1.00 = regime boosts expected returns.',
                  )),
                if (volAdj != null)
                  Expanded(child: _regimeStat(
                    'Vol Adj.',
                    '${volAdj.toStringAsFixed(2)}×',
                    tooltip: 'Volatility scaling factor applied to simulated price paths. >1.00 = higher modelled risk.',
                  )),
              ],
            ),
          ],
  ...
  Widget _yieldCurveCard() {
    ...
    final barColor   = isInverted ? const Color(0xFFEF4444) : const Color(0xFF26C96F);
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            ...
            style: TextStyle(color: barColor, fontSize: 22, fontWeight: FontWeight.w800),
          ),
          const SizedBox(height: 4),
          const Text('10-year minus 2-year Treasury yield',
          ...
          SizedBox(
            ...
            child: Row(
              ...
              children: values.map((v) {
                ...
                final col = v < 0 ? const Color(0xFFEF4444) : const Color(0xFF26C96F);
                return Expanded(
                  child: Padding(
          ...
          const Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text('12 months ago', style: TextStyle(color: _textDim, fontSize: 10)),
          ...
          if (isInverted) ...[
            ...
            Container(
              ...
              child: const Row(
                children: [
                  Icon(Icons.info_outline_rounded, color: Color(0xFFEF4444), size: 14),
                  SizedBox(width: 6),
                  Expanded(
                    child: Text(
        ...
        children: [
          const Row(
            children: [
              ...
              SizedBox(width: 8),
              Text('Fed & Economic Calendar',
                  style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 15)),
            ],
          ),
          const SizedBox(height: 14),
          if (_econCalendar.isEmpty)
            const Text('No upcoming high-impact events.',
  ...
  Widget _econEventTile(Map<String, dynamic> event) {
    ...
    Color impactColor;
    switch (impact) {
      case 'high':   impactColor = const Color(0xFFEF4444); break;
      case 'medium': impactColor = const Color(0xFFFFB547); break;
      default:       impactColor = _textDim;
    }
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                if (actual != null)
                  Text('Actual: $actual  Prev: ${previous ?? "-"}',
                      style: const TextStyle(color: _textDim, fontSize: 10))
                else if (forecast != null)
                  Text('Forecast: $forecast  Prev: ${previous ?? "-"}',
                      style: const TextStyle(color: _textDim, fontSize: 10)),
              ],
            ),
          ),
    ...
    return GlassCard(
      ...
      child: Row(
        children: [
          ...
          Text('$sectionName data temporarily unavailable',
...
class _ArcGaugePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final valuePaint = Paint()
      ...
      ..strokeWidth = 10
      ..strokeCap = StrokeCap.round;
    final sweepAngle = math.pi * (score / 100).clamp(0.0, 1.0);
    canvas.drawArc(rect, math.pi, sweepAngle, false, valuePaint);
    ...
    canvas.drawCircle(Offset(nx, ny), 5, dotPaint);
  }
